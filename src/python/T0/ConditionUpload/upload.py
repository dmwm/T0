...
defaultCookieFilename = 'cookie.txt'
...
class DropBox(object):
    ...
    def signInSSO(self, secure = True):
        '''Signs in the server via CERN SSO using Kerberos.
        '''

        if secure:
            logging.info('%s: Signing in via CERN SSO using Kerberos...', self.hostname)
        else:
            logging.info('%s: Signing in via CERN SSO using Kerberos (insecure)...', self.hostname)
    ...
    def signInSSOCert(self, certFilename, keyFilename, cookieFileName = defaultCookieFilename):
        '''Signs in the server via CERN SSO using a user certificate.

        Requires a certificate in the format accepted by cern-get-sso-cookie.
        '''

        # Use cern-get-sso-cookie and give the cookie to curl
        subprocess.check_call('cern-get-sso-cookie --cert %s --key %s -u %ssignInSSO -o %s' % (certFilename, keyFilename, self.urlTemplate % self.hostname, cookieFileName), shell = True)

        self.http.curl.setopt(self.http.curl.COOKIEFILE, cookieFileName)

        self.http.query('signInSSO')
...
def uploadTier0Files(filenames, username, password, cookieFileName = defaultCookieFilename):
