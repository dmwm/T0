...
def get_path_to_t0_root():
    """
    Work out the path to the T0 root from where the script is being run. Allows for
    calling setup.py env from sub directories and directories outside the T0 tree.
    """
        ...
        if recurse:
            ...
            for dirpath, dummy_dirnames, dummy_filenames in os.walk('%s' % a_dir, topdown=True):
                ...
                if len(set(pathelements) & ignore_these) == 0:
                    rel_path = os.path.relpath(dirpath, get_path_to_t0_root())
                    rel_path = rel_path.split('/')[2:]
                    packages.append('.'.join(rel_path))
                else:
                    print('Ignoring %s' % dirpath)
        else:
            rel_path = os.path.relpath(a_dir, get_path_to_t0_root())
            rel_path = rel_path.split('/')[2:]
            packages.append('.'.join(rel_path))
    return packages
...
def data_files_for(dir):
    ...
    if dir.endswith('+'):
        ...
        rel_path = os.path.relpath(dir, get_path_to_t0_root())
        add_static((rel_path.replace('src/', 'data/'), localfiles))
...
def list_static_files(system = None):
    ...
    if system:
        ...
        for language in ['couchapps+', 'css+', 'html+', 'javascript+', 'templates+']:
            static_files.extend(data_files_for('%s/src/%s' % (get_path_to_t0_root(), language)))
        for toplevel in ['bin+', 'etc+']:
            static_files.extend(data_files_for('%s/%s' % (get_path_to_t0_root(), toplevel)))
...
def things_to_build(command, pypi=False):
    ...
    for package in dependency_tree['packages']:
        ...
        src_path = '%s/src/python/%s' % (get_path_to_t0_root(), package.replace('.', '/'))
...
def print_build_info(command):
    ...
    if len(command.distribution.packages):
        ...
        for package in command.distribution.packages:
            if os.path.exists('%s/build/lib/%s' % (get_path_to_t0_root(), package.replace('.','/'))) and not command.force:
    ...
    if len(command.distribution.py_modules):
        ...
        for modules in command.distribution.py_modules:
            if os.path.exists('%s/build/lib/%s' % (get_path_to_t0_root(), modules.replace('.','/'))) and not command.force:
...
def force_rebuild():
    ...
    a clean sheet.
    """
    shutil.rmtree('%s/build' % get_path_to_t0_root(), True)
    shutil.rmtree('%s/doc/build' % get_path_to_t0_root(), True)
...
class BuildCommand(Command):
    ...
    def generate_docs (self):
        if not self.skip_docs:
            os.environ["PYTHONPATH"] = "%s/build/lib:%s" % (get_path_to_t0_root(), os.environ.get("PYTHONPATH", ''))
...
class InstallCommand(install):
    ...
    def finalize_options(self):
        ...
        docroot = "%s/doc/build/html" % get_path_to_t0_root()
